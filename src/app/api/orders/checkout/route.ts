...
import { checkStock, createOrder } from "../orderService";
import { getAuthUser, type AuthUser } from "@/lib/auth";
import { CartItem } from "@/hooks/useCart";
import { executeQuery } from "@/lib/db"; 

//配送費用
const SHIPPING_COST = 500; //500円固定
...
const stripe = new Stripe(SECRET_KEY, {
  apiVersion: "2026-06-24.dahlia", //最新のAPIバージョンを指定
});
  ...
  try {
    ...
    if (shortageItems.length > 0) {
      return NextResponse.json(
        { message: `在庫不足の商品があります：${shortageItems.join(", ")}` },
        { status: 400 },
      );
    }
    ...
    const line_items = items.map((item) => {
      //合計金額はデータベース情報を元にサーバー側で計算(改ざん対策)
    ...
    console.error("注文・決済エラー：", err);
    return NextResponse.json(
      { message: "注文登録または決済処理に失敗しました。" },
      { status: 500 },
    );
  }
}
