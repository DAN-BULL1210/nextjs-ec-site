...
export default function ProductEditPage() {
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    // 入力値のバリデーション（画僧ファイルなしは許容）
    if (!name.trim()) {
      setErrorMessage('商品名は必須です。');
      return;
    }
    if (price < 0 || stock < 0) {
      setErrorMessage('価格を在庫数は０以上の数値を入力してください。');
      return;
    }
    ...
    try { //商品編集APIにPUTリクエストを送信
      ...
      if (res.ok) { //更新成功時は管理者向け商品一覧ページへ
        router.push('/admin/products?edited=1'); //更新世校をクエリパラメータで通知  
      } else {
        const data = await res.json();
        setErrorMessage(data.message || '更新時に失敗しました。');
      }
    } catch {
      setErrorMessage('通信エラーが発生しました。');
    }
